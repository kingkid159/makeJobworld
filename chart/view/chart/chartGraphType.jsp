...
<body>
	...
	<div class="contain-bottom">
		...
		<c:forEach var="chartVO" items="${chartVO}">
				<img class='chartImg'
				src="${contextPath}/resources/image/시각화자료/${chartVO.job}_${chartVO.jobDetail}_${chartVO.graphType}.png" 
				alt="상세그래프"
				data-type='${chartVO.graphType}'/>
		</c:forEach>
		</div>
	</div>
...
<script>
	'use strict';
	var buttonContainer = document.querySelector('.buttonContain');
	var button = document.querySelectorAll('.graphTypeBTN');
	var imgContainer = document.querySelector('.imgContain');
	var charts = document.querySelectorAll('.chartImg');
	buttonContainer.addEventListener('click',(e)=>{
		var filter = e.target.dataset.filter;
		console.log(filter);
		imgContainer.classList.add('ani-out');
		setTimeout(() => {
			charts.forEach((chart)=>{
				if(filter ==='*' || filter === chart.dataset.type){
					chart.classList.remove('indivisible');
				}else{
					chart.classList.add('indivisible');
				}
			});
