...
url='https://edu.incruit.com/list/lecturelist.asp?ct=10&fld=05&occ1=150&sup=3&sortfield=1&sortorder=1&blocksize=60&page={}'
for p in range(1,3):
    result=requests.get(url.format(p))
    ...
    for url2 in urls:
        ...
        # print(eduphone)
        if eduphone.find('@')!=-1:  #이메일인경우
            eduphone=dom2.select_one('div.joview_table > table > tbody > tr:nth-of-type(5) > td > p').text
            # print(eduphone,'*************************')
        elif eduphone.find('-')==-1:     #전화번호도 아니고 이메일도 아닌경우
            # print(eduphone,'다시찾기전 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
            eduphone=dom2.select_one('#incruit_contents div.joview_table tr:nth-of-type(7) > td > p').text
            # print(eduphone,'다시찾은후 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
        acadamyadd = dom2.select_one(
            'div.jobpost_sider_cpinfo > div.jobpost_sider_cpinfo_table > dl:nth-of-type(1) > dd > p').text
        # print(acadamyadd)
        edufield = dom2.select_one(
            'div.jobpost_sider_jbinfo > div:nth-of-type(1) > dl:nth-of-type(1) > dd > div > div').text
        ...
        if  type(edulocation) == type(None):
            ...
            eduno = dom2.select_one(
                '#incruit_contents div.jobpost_top_cpinfo_new_2017 div.jobpost_sider_jbinfo > div:nth-of-type(1) > dl:nth-of-type(3) div.inset_ely_lay').text
            eduno = eduno.strip()
            # print(eduno)
            eduperiod = dom2.select_one(
                '#incruit_contents div.jobpost_top_cpinfo_new_2017 div.jobpost_sider_jbinfo > div:nth-of-type(1) > dl:nth-of-type(4) div.inset_ely_lay.letter_0').text
            eduperiod = eduperiod.strip()
            # print(eduperiod)
            edulocation = dom2.select_one(
                '#incruit_contents div.jobpost_top_cpinfo_new_2017 div.jobpost_sider_jbinfo > div:nth-of-type(1) > dl:nth-of-type(5) span.inblock').text
            # print(edulocation)
            eduprice = dom2.select_one(
                'div.jobpost_sider_jbinfo_inlay.jobpost_sider_jbinfo_inlay_last > dl:nth-of-type(1) div.inset_ely > div.inset_ely_lay > em.pt_txt.pt_txt_free').text
            # print(eduprice)
            eduplus = ' '
            # print(eduplus)
            eduapp = dom2.select_one(
                'div.inset_ely > div.inset_ely_lay > em.pt_txt.letter_0').text
            # print(eduapp)
        else:
            ...
            edulocation = dom2.select_one(
                '#incruit_contents div.jobpost_top_cpinfo_new_2017 div.jobpost_sider_jbinfo dl:nth-of-type(6) > dd > div.inset_ely > div.inset_ely_lay > span.inblock')
            edulocation = dom2.select_one(
                '#incruit_contents div.jobpost_top_cpinfo_new_2017 div.jobpost_sider_jbinfo dl:nth-of-type(6) > dd > div.inset_ely > div.inset_ely_lay > span.inblock').text
            edulocation = edulocation.replace(',', '')
            # print(edulocation)
            eduprice = dom2.select_one(
                '#incruit_contents div.jobpost_sider_jbinfo_inlay.jobpost_sider_jbinfo_inlay_last div.inset_ely em').text
            # print(eduprice)
            eduplus = dom2.select_one(
                '#incruit_contents div.jobpost_sider_jbinfo_inlay.jobpost_sider_jbinfo_inlay_last > dl:nth-of-type(2) div.inset_ely_lay').text
            ...
            # print(eduapp)
        # if eduapp==None:
        #     eduapp=dom.select_one('div.inset_ely > div.inset_ely_lay > em.pt_txt.letter_0').text
        #     print(eduapp)
        # print(
        #     '제목:{}\n이미지경로:{}\n학원명:{}\n문의 전화번호:{}\n학원주소:{}\n교육분야:{}\n교육형태:{}\n모집과정:{}\n모집인원:{}\n교육기간:{}\n교육지역:{}\n교육비용:{}\n수강특전:{}\n접수기간:{}\n'.format(
        #         title, imgsrc, acadamyname, eduphone, acadamyadd, edufield, edutype, eduprocess, eduno, eduperiod,
        #         edulocation, eduprice, eduplus, eduapp))
